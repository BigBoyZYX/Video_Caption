...
class DataHandler:
    ...
    def __init__(self,cfg,path,voc):
        ...
        if cfg.dataset == 'ml':
            self.train_dict, self.val_dict,self.test_dict = self._ml_create_dict() # Reference caption dictionaries
            # read appearance feature file
            self.appearance_feature_dict = self._read_feature_file(feature_type='appearance')
            self.appearance_test_feature_dict = self._read_feature_file(feature_type='appearance_test')
            # read motion feature file
            if cfg.model_name == 'marn':
                if cfg.opt_motion_feature:
                    self.motion_feature_dict = self._read_feature_file(feature_type='motion')
                    self.motion_feature_test_dict = self._read_feature_file(feature_type='motion_test')
                # read object feature file
                if cfg.opt_object_feature:
                    self.object_feature_dict = self._read_feature_file(feature_type='object')
                    self.object_test_feature_dict = self._read_feature_file(feature_type='object_test')


        self.train_name_list = list(self.train_dict.keys())
        self.val_name_list = list(self.val_dict.keys())
        test_file_list = os.listdir(path.test_video_path)
        self.test_name_list = []
        for file_name in test_file_list:
            self.test_name_list.append(file_name.split('.')[0])
    ...
    def _read_feature_file(self,feature_type='appearance'):
        ...
        elif feature_type == 'motion_test':
            f1 = h5py.File(self.path.motion_test_feature_file, 'r+')
        elif feature_type == 'appearance_test':
            f1 = h5py.File(self.path.appearance_test_feature_file,'r+')
        else:
            f1 = h5py.File(self.path.object_feature_file,'r+')
            
        if self.cfg.model_name == 'sa-lstm' or self.cfg.model_name == 'recnet' or self.cfg.model_name=='marn':
            for key in f1.keys():
                arr = f1[key][()] # modified by meihaiwei. ori f1[key].value
...
#         return vid2url
    ...
    def _ml_create_dict(self):
        ...
        test_dict = {}
        for datap in train_val_file['sentences']:
            if datap['video_id'] in self.path.train_id_list:
                if datap['video_id'] in list(train_dict.keys()):
                    train_dict[datap['video_id']] += [datap['caption']]
                else:
                    train_dict[datap['video_id']] = [datap['caption']]
            if datap['video_id'] in self.path.val_id_list:
                if datap['video_id'] in list(val_dict.keys()):
                    val_dict[datap['video_id']] += [datap['caption']]
                else:
                    val_dict[datap['video_id']] = [datap['caption']]
    ...
    def getDatasets(self):
        ...
        if self.cfg.model_name =='marn':
            ...
            val_dset = CustomDataset(self.cfg,self.appearance_feature_dict, self.val_dict, self.val_name_list, self.voc,
                                    self.motion_feature_dict,self.object_feature_dict)
            test_dset = CustomDataset(self.cfg,self.appearance_test_feature_dict, self.test_dict, self.test_name_list, self.voc,
                                     self.motion_feature_test_dict,self.object_feature_dict)
